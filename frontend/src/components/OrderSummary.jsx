  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            {cart.items.map((item) => (
              <div key={item._id} className="flex items-center justify-between border-b border-gray-200 py-2">
                <div className="flex items-center space-x-4">
                  <img src={item.product.imageUrl[0].url} alt={item.product.title} className="w-16 h-16 object-cover rounded-md" />
                  <div>
      ...
      <div className="flex justify-between items-center border-t border-gray-200 pt-4">
        <p className="text-sm font-medium">Total Items: {cart?.totalItems || 0}</p>
        <p className="text-sm font-medium">{`Total Price: $${cart?.totalPrice.toFixed(2) || 0}`}</p>
        <p className="text-sm font-medium">{`Total Discount: $${cart?.totalDiscount.toFixed(2) || 0}`}</p>
      </div>
