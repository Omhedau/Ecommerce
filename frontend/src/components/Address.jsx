...
import { listAddresses, createAddress, setShippingAddress, removeAddress } from '../redux/actions/address';
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        {addresses.length > 0 && (
          <button
            className={`px-4 py-2 rounded border ${!showNewAddressForm ? 'border-red-600 bg-red-600 text-white' : 'border-gray-400'} mr-4`}
        ...
        <button
          className={`px-4 py-2 rounded border ${showNewAddressForm ? 'border-red-600 bg-red-600 text-white' : 'border-gray-400'}`}
        ...
        <div className="grid grid-cols-1 gap-4">
          {loading ? (
            ...
            addresses.map((address) => (
              <div key={address._id} className="bg-gray-100 p-4 rounded border relative">
                ...
                <button
                  className="absolute top-1/2 right-4 transform -translate-y-1/2 bg-red-600 text-white rounded-full p-2 hover:bg-red-800"
                  onClick={() => handleRemoveAddress(address._id)}
                >
                  <MdDelete className="h-6 w-6" /> {/* MdDelete icon */}
                </button>
              </div>
            ))
          )}
